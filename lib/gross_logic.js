...
const GROSS_LOWER_BOUND_0_9_EVO = 6.169; // TARE LOWER + ( 0.9 / 1.05 ) // mode to use EVO and old
// const GROSS_UPPER_BOUND_0_9_EVO = 7.337; // TARE UPPER + ( 0.9 * 1.05 )
// const GROSS_LOWER_BOUND_0_9_EVO = 6.169; // TARE LOWER + ( 0.9 / 1.05 )
const GROSS_UPPER_BOUND_0_5_PF = 5.803; // TARE UPPER + ( 0.5 * 1.05 )
const GROSS_LOWER_BOUND_0_5_PF = 4.586; // TARE LOWER + ( 0.5 / 1.05 )
const GROSS_UPPER_BOUND_0_5_FLEX = 18.513; // TARE UPPER + ( 0.5 * 1.05 ) --> Inc.due to samples
const GROSS_LOWER_BOUND_0_5_FLEX = 17.594; // TARE LOWER + ( 0.5 / 1.05 )
...
async function collectGrosses() {
  ...
  while (currId <= endId) {
    ...
    // calc net
    let [currNet, expectedNet, percentError, inSpec] = await calcStats(
      currTare,
      currGross,
    );
    ...
    if (inSpec) {
      ...
      [currNet, expectedNet, percentError, inSpec] = await calcStats(
        currTare,
        cGross,
      );
...
async function validateGrossTolerance(tareIn) {
  ...
  if (process.env.CART_SIZE === "0.9") {
    return !(
      tareIn >= GROSS_LOWER_BOUND_0_9_EVO && tareIn <= GROSS_UPPER_BOUND_0_9_EVO
    );
  } else if (process.env.CART_SIZE === "0.5") {
    return !(
      tareIn >= GROSS_LOWER_BOUND_0_5_PF && tareIn <= GROSS_UPPER_BOUND_0_5_PF
    );
  } else if (process.env.CART_SIZE === "0.5_FLEX") {
    return !(
      tareIn >= GROSS_LOWER_BOUND_0_5_FLEX &&
      tareIn <= GROSS_UPPER_BOUND_0_5_FLEX
    );
  } else {
    return new Error(
...
async function correctGross(currTare, currGross) {
  ...
  while (!inSpec) {
    let [currNet, expectedNet, percentError, newSpec] = await calcStats(
      currTare,
      currGross,
    );
    menu.clearScreen();
...
async function calcStats(currTare, currGross) {
  ...
  let expectedNet = parseFloat(process.env.CART_SIZE);
  let percentError = (
    Math.abs((currNet - expectedNet) / expectedNet) * 100
  ).toFixed(2);
