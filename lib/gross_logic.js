...
async function collectGrosses() {
  ...
  while (currId <= endId) {
    ...
    console.log(Number(process.env.CART_SIZE));

    // get new gross value
    let currGross = rl.question('Enter current gross: ');
    ...
    // display results
    menu.clearScreen();
    menu.printGrossStats(currNet, percentError, inSpec);
...
async function correctGross(currTare, currGross) {
  ...
  while (!inSpec) {
    ...
    menu.clearScreen();
    menu.printGrossStats(currNet, percentError, newSpec);
